...
/**
 * Initialize a new test `Suite` with specified options.
 *
 * The test Suite inherits from EventEmitter.
 *
...
function Suite(options) {
    ...
    /**
     * The number of milliseconds to show a progress status.
     * @type {Number}
     */
    this.progressInterval = (options.progressInterval) ? options.progressInterval : null;
 ...
 * @property {Number} [progressInterval] The number of milliseconds to update the progress status. When `0` or `null` disables
 * progress reporting. Defaults to `0`.
 */
...
Suite.prototype.run = function (done) {
    ...
    if (this.progressInterval) {
        this._progressEmitter = setInterval(function () {
            this.emit('progress', this._currentIteration, this.iterations);
        }.bind(this), this.progressInterval);
    }
    async.waterfall(this._beforeAll, function (err) {
        ...
        this._pushNext(q);
        q.drain = function () {
            if (this._progressEmitter) { // Stop emitting progress event
                clearInterval(this._progressEmitter);
                this._progressEmitter = null;
            }
